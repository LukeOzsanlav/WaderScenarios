...
CreateScenario <- function(Canvas, 
  ...
  for(i in 1:N_sets){ 
    ...
    HabCats <- c("Category", "GrassOpp", "GRASSLAND_TYPE", "WaterCoverage", "STOCK", "VEG_STRUCTURE", "RUSH_PERCENT", "Fence_Coverage")
    ...
    ## WHen this is raterized it could miss some of th arable fields if Opp is not updated to "Grass"
    ...
    if(Unfenced==TRUE){
      ...
      if(is.null(SniModel)==F){
        Pred <- (predict.rfsrc(object = SniModel, 
                               newdata = (Canvas[Index, ] |> select(SniModel[["xvar.names"]]) |> st_drop_geometry() |> as.data.frame() |> mutate(Fence_Coverage = Fence_CoverageOrig)), 
                               jitt=FALSE))$predicted
      ...
      if(is.null(LapModel)==F){
          Pred <- (predict.rfsrc(object = LapModel, 
                                  newdata = (Canvas[Index, ] |> select(LapModel[["xvar.names"]]) |> st_drop_geometry() |> as.data.frame() |> mutate(Fence_Coverage = Fence_CoverageOrig)), 
                                  jitt=FALSE))$predicted
      ...
      if(is.null(RedModel)==F){
          Pred <- (predict.rfsrc(object = RedModel, 
                                  newdata = (Canvas[Index, ] |> select(RedModel[["xvar.names"]]) |> st_drop_geometry() |> as.data.frame() |> mutate(Fence_Coverage = Fence_CoverageOrig)), 
                                  jitt=FALSE))$predicted
